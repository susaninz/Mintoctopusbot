...
class GPTService:
    ...
    def analyze_master_profile(self, profile_text: str) -> Dict:
        ...
        # Получаем текущую информацию о времени в московском часовом поясе
...
ТЕКУЩЕЕ ВРЕМЯ (Москва): {time_info['current_moscow_time']}
...
Правила для временных слотов (используй МОСКОВСКОЕ время):
- Если указано "завтра" - используй {time_info['tomorrow_date']}
- Если указано "сегодня" - используй {time_info['current_date']}
- Если указано "вторник 29" - это 29 июля 2025
- Если указано "среда" без даты - это ближайшая среда от {time_info['current_date']}
...
Правила для дат:
1. "Сегодня" = {time_info['current_date']}
2. "Завтра" = {time_info['tomorrow_date']}
3. Дни недели ищи от текущей даты
